...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	//    upstream 的 ApplyOtherRatiosToFloat() 不过滤 "_" 前缀诊断键，必须手动过滤，
	//    否则 _doubao_resolution: 1080 等诊断值会被乘入 quota 导致计费异常放大。
	//    同时采用 upstream 的饱和转换（QuotaFromFloatChecked）防止溢出成负数。
	if !common.StringsContains(constant.TaskPricePatches, modelName) {
		multiplier := 1.0
		for k, ra := range info.PriceData.OtherRatios() {
			if ra != 1.0 && ra > 0 && !strings.HasPrefix(k, "_") {
				multiplier *= ra
			}
		}
